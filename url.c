...
int parse_url(char* url, url_info *info)
{
	...
	char *column_slash_slash, *host_name_path, *protocol,*trainling_slash,*port;
	 ...
	 */
	trainling_slash=strchr(host_name_path,'/');
	if (trainling_slash==NULL){
		return PARSE_URL_NO_SLASH;
	} 
	*trainling_slash='\0';
	info->host=host_name_path;
	info->path=trainling_slash+1;
